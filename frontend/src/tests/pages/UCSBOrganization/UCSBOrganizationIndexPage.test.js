...
describe("UCSBOrganizationIndexPage tests", () => {
    ...
    test("does not render Create Button for non-admin user", async () => {
        ...
        await waitFor(() => { expect(screen.queryByText(/Create Your UCSB Organization/)).not.toBeInTheDocument(); });
    
        const createButton = screen.queryByText(/Create Your UCSB Organization/);
    ...
    test("Renders with Create Button for admin user", async () => {
        ...
        await waitFor(() => {
            expect(screen.getByText(/Create Your UCSB Organization/)).toBeInTheDocument();
        });
        const button = screen.getByText(/Create Your UCSB Organization/);
    ...
    test("renders three UCSBOrganizations correctly for regular user", async () => {
        ...
        await waitFor(() => { expect(screen.getByTestId(`${testId}-cell-row-0-col-orgCode`)).toHaveTextContent("DS"); });
        ...
        const orgCode = screen.getByText("DS");
        expect(orgCode).toBeInTheDocument();

        const orgTranslationShort = screen.getByText("Delta Sig");
        expect(orgTranslationShort).toBeInTheDocument();
        const orgTranslation = screen.getByText("Delta Sigma Pi");
    ...
    test("what happens when you click delete, admin", async () => {
        ...
        axiosMock.onGet("/api/UCSBOrganization/all").reply(200, ucsbOrganizationFixtures.threeOrganizations);
        axiosMock.onDelete("/api/UCSBOrganization").reply(200, "UCSBOrganization with orgCode DS was deleted");
        ...
        await waitFor(() => { expect(mockToast).toBeCalledWith("UCSBOrganization with orgCode DS was deleted") });

        await waitFor(() => { expect(axiosMock.history.delete.length).toBe(1); });
        expect(axiosMock.history.delete[0].url).toBe("/api/UCSBOrganization");
        expect(axiosMock.history.delete[0].url).toBe("/api/UCSBOrganization");
        expect(axiosMock.history.delete[0].params).toEqual({ orgCode: "DS" });
    });
