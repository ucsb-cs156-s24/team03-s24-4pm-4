...
describe("UCSBOrganizationTable tests", () => {
  ...
  test("validates data accessors are functioning correctly", () => {
    render(
      <QueryClientProvider client={queryClient}>
        <MemoryRouter>
          <UCSBOrganizationTable organizations={ucsbOrganizationFixtures.threeOrganizations} currentUser={currentUserFixtures.adminUser} />
        </MemoryRouter>
      </QueryClientProvider>
    );

    // Check if all rows and relevant columns contain correct data based on accessors
    ucsbOrganizationFixtures.threeOrganizations.forEach((org, index) => {
      const orgCodeNode = screen.getByTestId(`${testId}-cell-row-${index}-col-orgCode`);
      expect(orgCodeNode).toHaveTextContent(org.orgCode);

      const orgTranslationNode = screen.getByTestId(`${testId}-cell-row-${index}-col-orgTranslation`);
      expect(orgTranslationNode).toHaveTextContent(org.orgTranslation);

      const orgTranslationShortNode = screen.getByTestId(`${testId}-cell-row-${index}-col-orgTranslationShort`);
      expect(orgTranslationShortNode).toHaveTextContent(org.orgTranslationShort);

      const currentlyInactiveNode = screen.getByTestId(`${testId}-cell-row-${index}-col-Currently Inactive`);
      expect(currentlyInactiveNode).toHaveTextContent(String(org.inactive));
    });
  });
